...
function CreditsWidget({onTopUp}: {onTopUp: () => void}) {
  ...
  return (
    <div className={`rounded-2xl border p-4 ${hasDebt ? 'bg-red-950/40 border-red-500/30' : isLow ? 'bg-amber-950/40 border-amber-500/30' : 'bg-gradient-to-br from-amber-500/12 to-yellow-500/8 border-amber-500/25'}`}>
      {/* Balance display */}
      ...
      {loaded ? (
        <p className={`text-3xl font-bold tabular-nums mt-1 ${hasDebt ? 'text-red-300' : isLow ? 'text-amber-300' : 'text-amber-200'}`}>
      ...
      <button
        type="button"
        onClick={onTopUp}
        className={`mt-4 w-full rounded-xl py-2.5 text-sm font-bold flex items-center justify-center gap-2 transition-all ${
          hasDebt
...
function CourseRow({
  ...
  return (
    <div className="flex items-center gap-3 rounded-2xl border border-white/6 bg-white/[0.02] px-4 py-3 hover:bg-white/[0.04] transition-colors group">
      {/* Thumbnail */}
      <img src={course.imageUrl} alt={course.imageAlt} className="w-12 h-12 rounded-xl object-cover border border-white/8 shrink-0" />
      ...
      {enrollment ? (
        <Link
          ...
          className="shrink-0 rounded-xl bg-white/8 border border-white/10 px-3 py-1.5 text-xs font-medium hover:bg-white/15 transition-colors inline-flex items-center gap-1.5 whitespace-nowrap"
        >
        ...
        <button
          ...
          onClick={onEnroll}
          disabled={enrolling}
          className="shrink-0 rounded-xl border border-white/15 px-3 py-1.5 text-xs font-medium hover:bg-white hover:text-black transition-colors disabled:opacity-40 inline-flex items-center gap-1.5 whitespace-nowrap"
        >
  ...
  return (
    <div className="rounded-2xl border border-white/8 bg-white/[0.025] p-4">
      <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between mb-4">
        <div>
          <p className="text-[10px] uppercase tracking-[0.24em] text-white/35 mb-1">Studio command</p>
          <h3 className="text-lg font-serif">{nextModuleTitle ?? course.title}</h3>
          <p className="text-xs text-white/45 mt-1">Jump into the exact tool you need for the next study block.</p>
        </div>
        <Link
          to={`/courses/${course.id}`}
          className="rounded-full bg-white text-black px-4 py-2 text-xs font-semibold hover:bg-gray-200 transition-colors inline-flex items-center justify-center gap-1.5"
        >
          Open studio
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-2">
        {shortcuts.map(({label, Icon, tint, detail}) => (
          <Link
            key={label}
            to={`/courses/${course.id}`}
            className="rounded-2xl border border-white/8 bg-white/[0.03] p-3 hover:bg-white/[0.06] transition-colors group"
          >
...
function MobilePulse({
  ...
  return (
    <div className="xl:hidden grid grid-cols-3 gap-2 p-4 border-b border-white/6">
      ...
      <button
        type="button"
        onClick={onTopUp}
        className="col-span-3 rounded-2xl border border-amber-400/25 bg-amber-400/10 px-4 py-3 text-xs font-semibold text-amber-300 inline-flex items-center justify-center gap-2"
      >
        <CreditCard className="w-3.5 h-3.5" />
        Manage credits and upcoming sessions
      </button>
    </div>
  );
}
...
export default function Dashboard() {
  ...
  return (
    <div className="h-full grid grid-cols-1 xl:grid-cols-[220px,1fr,272px] overflow-hidden">

      {/* ══════════════════════════════════════
          LEFT SIDEBAR — identity + stats + nav
      ══════════════════════════════════════ */}
      <aside className="hidden xl:flex flex-col border-r border-white/8 overflow-y-auto">

        {/* User identity */}
        <div className="p-5 border-b border-white/6">
          <div className="flex items-center gap-3 mb-3">
            <div className="w-10 h-10 rounded-2xl liquid-glass border border-white/15 flex items-center justify-center shrink-0">
              <span className="text-base font-bold text-white/90">{firstName[0]?.toUpperCase()}</span>
            </div>
        ...
        <div className="p-4 border-b border-white/6 space-y-2">
          <p className="text-[10px] uppercase tracking-[0.22em] text-white/30 mb-3">Progress</p>
          {[
            ...
            <div key={label} className="flex items-center justify-between py-1.5 px-3 rounded-xl bg-white/[0.03] border border-white/5">
        ...
        <div className="p-4 border-b border-white/6">
          <p className="text-[10px] uppercase tracking-[0.22em] text-white/30 mb-3">Navigate</p>
        ...
        <div className="p-4 flex-1">
          <p className="text-[10px] uppercase tracking-[0.22em] text-white/30 mb-3">Your profile</p>
      ...
      <div className="flex flex-col overflow-hidden">
        ...
        {/* Focus hero strip */}
        <div className="shrink-0 p-5 border-b border-white/6 liquid-glass relative overflow-hidden">
          ...
          <div className="relative">
            ...
            <h2 className="text-xl font-serif tracking-tight leading-snug mb-3">
              {firstName},
              {nextFocusRow
            ...
            <div className="flex flex-wrap gap-2">
              {nextFocusRow ? (
                <Link
                  ...
                  className="bg-white text-black rounded-full px-4 py-2 text-xs font-semibold hover:bg-gray-200 transition-colors inline-flex items-center gap-1.5"
                >
              ...
              <Link
                to="/profile"
                className="rounded-full px-4 py-2 text-xs border border-white/15 text-white/65 hover:text-white hover:border-white/30 transition-colors"
              >
                Refine profile
              </Link>
            </div>
        ...
        {/* Search + tabs */}
        <div className="shrink-0 px-5 pt-4 pb-3 border-b border-white/6">
      ...
      ══════════════════════════════════════ */}
      <aside className="hidden xl:flex flex-col border-l border-white/8 overflow-y-auto p-4 gap-4">
        ...
        <div className="rounded-2xl border border-white/6 bg-gradient-to-br from-indigo-500/8 to-violet-500/5 p-4">
